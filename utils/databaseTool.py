...
class MrcDatabase(object):
    ...
    def __init__(self, sqlitePath:str):
        ...
        self.__maxArticleNumber = self.getMaxArticleNumber()
    ...
    def insertQuestionAnswer(self, articleId:int, question:str, answerStart:int, answerString:str):
        ...
        with self.__conn:
            ...
            cursor.execute(query, (articleId, question, answerString, answerStart, timestamp))
    ...
    def insertArticle(self, question:str, context:str):
        query = 'INSERT into article (question, context, timestamp) VALUES (?, ?, ?);'
        timestamp = datetime.datetime.now().replace(tzinfo=self.__timezone).isoformat()
        with self.__conn:
            cursor = self.__conn.cursor()
            cursor.execute(query, (question, context, timestamp))
    ...
    def getQuestionById(self,idx:int) -> str:
        if idx > self.__maxArticleNumber:
            result = 'ERROR: <ArticleOutOfBound>'
        else:
            query = 'SELECT question FROM article WHERE id = ?'
            with self.__conn:
                cursor = self.__conn.cursor()
                cursor.execute(query,(idx,))
                result = cursor.fetchone()[0]
    ...
    def getQuestionAndAnswerByArticleId(self, articleId:int) -> (str, int, str):
        if articleId > self.__maxArticleNumber:
            ...
            query = 'SELECT question, answer_string, answer_start FROM question_answer WHERE article_id = ?'
            with self.__conn:
                cursor = self.__conn.cursor()
    ...
    def getQuestionAndAnswerById(self, idx:int) -> (str, int, str):
        if idx > self.__maxQuestionAndAnswerNumber:
            ...
            query = 'SELECT question, answer_string, answer_start FROM question_answer WHERE id = ?'
            with self.__conn:
                cursor = self.__conn.cursor()
                cursor.execute(query, (idx,))
                result = cursor.fetchone()[0]
    ...
    def getAllQuestionAndAnswer(self):
        query = 'SELECT question, answer_string, answer_start FROM question_answer'
        with self.__conn:
            cursor = self.__conn.cursor()
            cursor.execute(query)
            result = cursor.fetchall()
